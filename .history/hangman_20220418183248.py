...
def get_a_word(wordlist):
    hangmanWord = random.choice(wordlist)
    while '-' in wordlist or ' ' in wordlist:
        handmanword = random.choice(wordlist)
        
        return hangmanWord 

#function to keep track of words that are guessed and not guessed correctly 
def hangman():
    theword = get_a_word(wordlist)
    letters_in_word = set(theword) #will define letters in the word 
...
#user input 
    ...
    #what is the current word they are guessing(ex: l _ V _)
        wordlist = [letter if letter in user_letter else '-' for letter in wordsList]
        print('The current word: ',' '.join(wordlist))
